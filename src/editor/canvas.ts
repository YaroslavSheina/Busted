...
import { drawSigns, arrowSpots, branchSide, groundColor, setCity, setClip, drawBlocks, drawCar, drawCrossingRoad, drawCrossingTop, drawGrid, drawPolice, drawProps, drawRails, drawRamp, drawRoad, drawTurnArrow } from '../render';
...
export function initCanvas(cv: HTMLCanvasElement, h: CanvasHooks): EditorCanvas {
  ...
  function draw(): void {
    ...
    if (path) {
      ...
      drawSigns(ctx, path, wMain, l.signs);
      (l.branches ?? []).forEach((b, bi) => {
