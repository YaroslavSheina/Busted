    ...
    routes: [
      ...
      { file: 'final', name: 'Финал', car: 'finale', speed: 420, traffic: 0.2, mix: 0.1, panic: 0.4, chaser: { gap: 260, speed: 1.02 }, chaserAt: [[7, -1.5], [3, -2.0], [3, -10.5]], // gap 260: к первому переезду коп подтягивается до ~0.48 с и попадает под поезд (after 0.3)
        ...
        events: [
          ...
          { type: 'works', at: [5, -2.8], lanes: [2], len: 200 }, // на спуске, между перекрёстками
          ...
          { type: 'post', at: [3, -10.6], lanes: [1, 2] }, // за съездом с кольца (3,−9), до перекрёстка (3,−11)
        ...
        roads: [
          ...
          // кольцо налево на (3,−9) — финал проходит все стороны экрана на полной скорости (docs/teaching.md, §6)
          { nodes: [[7, 0], [7, -4], [5, -4], [5, -1], [3, -1], [3, -6], [1, -6], [1, -9], [3, -9], [3, -12], [7, -12], [7, -10]], rings: [[3, -9]], oncoming: 0, offsets: 'green', cars: () => [] },
        ] },
    ] },
  ...
  { seed: 6303, car: 'sport', speed: 350, traffic: 0.4, panic: 0.3, mix: 0.15, chaser: { gap: 200, speed: 1.02 }, grid: { bx: 600, by: 520, road: 180, r: 200 }, blocks: { i: [-1, 7], j: [-13, 0] },
    routes: [
      { file: 'night1', name: 'Ночной город · 1', chaser: null, traffic: 0.35, intro: 'О тебе говорят. Спорт: острый руль, быстро гасит вираж. Улицы тесные, слева встречка, и полиция уже знает номер.',
        // спуск (2,−4)→(2,−2) по двусторонней улице (docs/teaching.md, §6)
        events: [{ type: 'post', at: [0, -2.5] }, { type: 'works', at: [4, -4.4], lanes: [2], len: 150 }, { type: 'narrow', at: [6, -7.4], to: [6, -7.7], width: 120 }],
        roads: [{ nodes: [[0, 0], [0, -4], [2, -4], [2, -2], [4, -2], [4, -6], [6, -6], [6, -10]], oncoming: 1, reds: [[0, -3], [4, -5]] }] },
      ...
      // «Дождь» — кольцо налево на мокрой дороге с попутным трафиком (docs/teaching.md, §6): на кольце одностороннее движение
      { file: 'night_wet', name: 'Ночной город · Дождь', traffic: 0.3, chaser: null, grip: 0.7, intro: 'Дождь. Сцепление ниже — машину несёт дольше. Поворачивай раньше, чем привык. Впереди кольцо: на мокром круге держи машину нежно',
        events: [{ type: 'post', at: [4, -2.5] }, { type: 'works', at: [2, -6.4], lanes: [2], len: 150 }, { type: 'narrow', at: [0, -10.4], to: [0, -10.7], width: 120 }],
        roads: [{ nodes: [[4, 0], [4, -4], [2, -4], [2, -9], [0, -9], [0, -12]], rings: [[2, -9]], oncoming: 0, reds: [[2, -6], [0, -11]] }] },
      // «змейка» (docs/teaching.md, §6): вверх-вниз через квартал — кнопки то так, то наоборот; второй коп появляется на спуске
      { file: 'night2', name: 'Ночной город · 2', chaserAt: [[0, -0.5], [3, -3.4]], intro: 'Ночной город · 2. Змейка: вверх, вниз, вверх — кнопки то так, то наоборот. Коп с самого старта, второй — на спуске. Переезды — твоё оружие.',
        events: [{ type: 'works', at: [0, -1.5], lanes: [2], len: 120 }, { type: 'rails', at: [2, -2.6], offset: 'behind', length: 500 },
          { type: 'rails', at: [4, -3.6], offset: 'behind', length: 500 }],
        roads: [{ nodes: [[0, 0], [0, -3], [1, -3], [1, -1], [2, -1], [2, -4], [3, -4], [3, -2], [4, -2], [4, -5], [5, -5], [5, -3]], oncoming: 1, reds: [[1, -2], [4, -4]] }] },
